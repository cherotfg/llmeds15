...
function renderItems(block, items, bridge) {
  ...
  list.forEach((item, i) => {
    ...
    imageContainer.className = 'browse-solutions-image';
  ...
  leftBtn.className = 'browse-solutions-arrow browse-solutions-arrow-left';
  leftBtn.type = 'button';
  leftBtn.setAttribute('aria-label', 'Scroll left');
  leftBtn.textContent = '◀';

  const rightBtn = document.createElement('button');
  rightBtn.className = 'browse-solutions-arrow browse-solutions-arrow-right';
  rightBtn.type = 'button';
  rightBtn.setAttribute('aria-label', 'Scroll right');
  rightBtn.textContent = '▶';

  const CARD_STEP = 236; // 220px card + 16px gap
  const scrollBy = (dir) => track.scrollBy({ left: dir * CARD_STEP, behavior: 'smooth' });
